...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/kizashi/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/kizashi/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/kizashi/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^kizashi_") ; then
        LINEAGE_BUILD=$(echo -n $1 | sed -e 's/^kizashi_//g')
    else
        LINEAGE_BUILD=
    fi
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the kizashi GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/kizashi/build/tools/roomservice.py $product
        cd - > /dev/null
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/kizashi/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
. $ANDROID_BUILD_TOP/vendor/kizashi/build/envsetup.sh
